...
export async function getPelicanServer(
	baseUrl: URL,
	serverID: string,
	headers: HeadersInit,
): Promise<PelicanServer> {
	{
	...
	const server = new PelicanServer(ws, async () => (await getJWT()).jwt);
		...
		ws.on('message', async (msg) => {
			...
			switch (event) {
				...
				case 'stats':
					...
					break;
				default:
					console.warn('[Pelican] Unknown message type. Report this to the package maintainers.', {
						event,
						args,
					});
			}
